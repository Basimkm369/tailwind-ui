...
function FeaturePill({ label, status }) {
  return (
    <div className={`flex items-center gap-3 rounded-full px-3 py-2 text-sm ${pillStyles[status]}`}>
...
function TicketCard({ ticket }) {
  ...
  return (
    <div className="relative card-border overflow-hidden rounded-[22px] shadow-card min-h-[320px]">
      <div className="absolute inset-0 card-shine" style={{ backgroundImage: `${glowLayers}, ${ticket.gradient}` }} />
      <div className="absolute inset-0 bg-gradient-to-b from-black/15 via-black/15 to-black/45" />
      {ticket.badge && (
        <div className={`ribbon ${ticket.badge.color === 'orange' ? 'orange' : ''}`}>{ticket.badge.text}</div>
      )}
      ...
      <div className="relative flex h-full flex-col gap-4 px-5 py-5 sm:px-6 sm:py-6">
        <div>
          <p className="text-[12px] font-semibold uppercase tracking-[0.06em] text-white/90">Visitor 3 Day Access Ticket</p>
          <p className="text-[12px] font-semibold uppercase tracking-[0.04em] text-white/70">{ticket.sub}</p>
        </div>

        <div className="flex flex-col gap-3">
          <p className="text-xs text-white/70 leading-relaxed">
            Visitor Passes provide <span className="font-semibold text-[#39d86c]">3 DAYS ACCESS</span> to GITEX NIGERIA exhibition and all free
            conference
          </p>
          {ticket.logoText && (
          ...
          <div className="grid grid-cols-1 gap-2">
            {features.map((item) => (
              <FeaturePill key={item.key} label={item.label} status={item.status} />
            ))}
          </div>
        </div>
          ...
          <div className="flex items-end gap-2 text-white">
            {ticket.price.currency !== 'FREE' ? (
              <>
                ...
                <span className="text-3xl font-bold leading-none drop-shadow-sm">{ticket.price.amount}</span>
              ...
              <div>
                <div className="text-lg font-bold leading-tight">{ticket.price.currency}</div>
                <div className="text-[11px] uppercase text-white/80">{ticket.price.note}</div>
              </div>
            )}
          </div>
          {ticket.toggles ? (
...
function App() {
  return (
    <div className="min-h-screen bg-gradient-to-b from-transparent via-white to-transparent">
      <div className="relative overflow-hidden bg-hero-gradient text-white shadow-card">
        <div className="pixel-cluster absolute left-4 top-4 h-40 w-32" />
        <div className="pixel-cluster absolute left-16 top-10 h-28 w-24 opacity-50" />
        <div className="pixel-cluster absolute right-4 top-4 h-20 w-20 opacity-0" />
        <div className="mx-auto max-w-6xl px-6 py-8">
          <div className="flex flex-col gap-2">
            <p className="text-[14px] font-semibold tracking-[0.08em]">VISITOR PASSES PROVIDE</p>
            <h1 className="text-[34px] font-bold leading-tight">ACCESS TO GITEX NIGERIA EXHIBITION AND</h1>
            <p className="text-[18px] font-semibold">ALL FREE CONFERENCE</p>
          </div>
        </div>
      </div>

      <main className="relative z-10 mx-auto -mt-10 max-w-6xl px-4 pb-12">
      ...
      <footer className="relative bg-footer-gradient text-white">
        <div className="pixel-cluster absolute left-4 bottom-6 h-36 w-28" />
        <div className="pixel-cluster absolute left-24 bottom-2 h-24 w-24 opacity-60" />
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-4 px-6 py-6 sm:flex-row">
          <div className="text-[13px] font-semibold uppercase tracking-[0.06em]">View Details →</div>
          <div className="flex flex-1 flex-wrap items-center justify-center gap-4 sm:justify-end">
            <div className="text-center sm:text-right">
              <div className="text-xl font-bold">
                Total: <span className="text-[#e3ff92]">EUR 0</span>
              </div>
              <div className="text-[12px] uppercase text-white/85">Incl. 19% VAT</div>
              <div className="text-[11px] text-white/80">View Ticket summary</div>
            </div>
            <button className="rounded-xl bg-white px-6 py-3 text-sm font-semibold uppercase tracking-wide text-gray-900 shadow-md transition hover:-translate-y-[1px] hover:shadow-lg">
              Buy Now
            </button>
          </div>
        </div>
